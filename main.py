...
def receive_scan(scan: ScanIn, session: Session = Depends(get_session)):
    ...
    if not goods.registered:
        # Tag hasn't been named/registered yet — log the scan but don't move stock.
        ...
        log = ScanLog(
            uid=scan.uid,
            action=ScanAction.unregistered,
            quantity_change=0,
            resulting_quantity=goods.quantity,
        ...
        return {
            ...
            "low_stock": False,
            "message": "Tag scanned but not registered yet — register it from the dashboard to start tracking stock.",
        }
